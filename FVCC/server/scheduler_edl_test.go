...
func (f *fakeDispatcher) CreateRenderEDL(server Server, t Task) (string, error) {
	return f.CreateRenderEDLWithTrace(server, t, t.TraceID)
}

func (f *fakeDispatcher) CreateGenProxy(server Server, t Task) (string, error) {
	return f.CreateGenProxyWithTrace(server, t, t.TraceID)
}

func (f *fakeDispatcher) CreateRenderEDLWithTrace(server Server, t Task, traceID string) (string, error) {
	f.edlCalls++
...
func (f *fakeDispatcher) CreateGenProxyWithTrace(server Server, t Task, traceID string) (string, error) {
	f.proxyCalls++
	f.lastServer, f.lastTask = server, t
	if f.err != nil {
		return "", f.err
...
func newSchedTestEnv(t *testing.T) (*Scheduler, *Store) {
	t.Helper()
	...
	return NewScheduler(s, NewRemoteClient(), NewHub(), nil), s
}

func mustTask(t *testing.T, s *Store, id string) Task {
	t.Helper()
	got, ok := s.GetTask(id)
	if !ok {
...
func TestB05RenderDispatchSuccess(t *testing.T) {
	...
	s.UpsertTask(Task{
		ID: "t_edl", OrderID: 1, Status: StatusQueue, TaskType: TaskTypeRenderEDL,
	...
	if got.Status != StatusTranscoding {
	...
	if d.lastTask.PayloadJSON == "" || d.lastTask.TaskType != TaskTypeRenderEDL {
...
func TestB05GenProxyDispatchAndLowPriority(t *testing.T) {
	...
	s.UpsertTask(Task{ID: "t_proxy", OrderID: 1, Status: StatusQueue, TaskType: TaskTypeGenProxy,
		ServerID: "srv1", PayloadJSON: `{"assetId":"a1"}`})
	sch.handleGenProxy(mustTask(t, s, "t_proxy"))

	if got := mustTask(t, s, "t_proxy"); got.Status != StatusTranscoding || got.RemoteTaskID != "remote_proxy_1" {
	...
	queued := sortQueueTasks([]Task{
		{ID: "t_proxy", OrderID: 100, Status: StatusQueue, TaskType: TaskTypeGenProxy},
		{ID: "t_render", OrderID: 200, Status: StatusQueue, TaskType: TaskTypeRenderEDL},
		{ID: "t_trans", OrderID: 300, Status: StatusQueue, TaskType: TaskTypeTranscode},
	})
	...
	// 同档位按 OrderID 升序。
	sameRank := sortQueueTasks([]Task{
		{ID: "t_b", OrderID: 20, Status: StatusQueue, TaskType: TaskTypeRenderEDL},
		{ID: "t_a", OrderID: 10, Status: StatusQueue, TaskType: TaskTypeRenderEDL},
	})
...
func TestB05CooldownBackoffSequence(t *testing.T) {
	...
	s.UpsertTask(Task{ID: "t_bk", OrderID: 42, Status: StatusQueue, TaskType: TaskTypeRenderEDL,
	...
	for i, want := range wantCool {
		...
		if got.Status != StatusCooldown || got.Status.WireName() != "COOLDOWN" {
		...
		if got.CooldownReason != errCodeNodeOffline {
			t.Fatalf("cooldown_reason 应为 %s，实际 %s", errCodeNodeOffline, got.CooldownReason)
		}
		if got.OrderID != 42 {
		...
		if still := mustTask(t, s, "t_bk"); still.Status != StatusCooldown {
	...
	if over.Status != StatusError || over.Status.WireName() != "FAILED" {
		t.Fatalf("重试次数用尽应转 FAILED，实际 %s", over.Status)
	}
	if over.RetryType != NonRetryable || over.CoolDownUntil != nil {
...
func TestB05ErrorCodeWhitelist(t *testing.T) {
	nonRetryable := []string{errCodeEDLInvalid, errCodeAssetMissing, errCodeDiskFull, errCodeFFmpegMissing}
	for _, code := range nonRetryable {
		sch, s := newSchedTestEnv(t)
		sch.SetRenderDispatcher(&fakeDispatcher{err: errors.New(code + ": 用例构造")})
		s.UpsertTask(Task{ID: "t_nr", OrderID: 1, Status: StatusQueue, TaskType: TaskTypeRenderEDL,
			ServerID: "srv1", PayloadJSON: `{"clips":[]}`})

		sch.handleRenderEDL(mustTask(t, s, "t_nr"))
		got := mustTask(t, s, "t_nr")
		if got.Status != StatusError || got.RetryType != NonRetryable {
	...
	for _, tc := range []struct{ err, wantCode string }{
		{"E_SMB_MOUNT_FAILED: 挂载失败", errCodeSMBMountFailed},
		{"E_TIMEOUT_STALL: 无进度", errCodeTimeoutStall},
		{"E_RENDER_FAILED: ffmpeg 非零退出", errCodeRenderFailed},
		{"网络抖动", errCodeRenderFailed},
	} {
...
func TestB05NodeGuards(t *testing.T) {
	...
	d := &fakeDispatcher{}
	sch.SetRenderDispatcher(d)
	s.UpsertTask(Task{ID: "t_off", OrderID: 1, Status: StatusQueue, TaskType: TaskTypeRenderEDL,
		ServerID: "srv_off", PayloadJSON: `{"clips":[]}`})
	sch.handleRenderEDL(mustTask(t, s, "t_off"))
	off := mustTask(t, s, "t_off")
	if off.Status != StatusCooldown || off.CoolDownSec != 30 || off.CooldownReason != errCodeNodeOffline {
	...
	// 节点不存在 → 直接失败（不可重试）。
	s.UpsertTask(Task{ID: "t_ghost", OrderID: 2, Status: StatusQueue, TaskType: TaskTypeRenderEDL,
		ServerID: "srv_missing", PayloadJSON: `{"clips":[]}`})
	sch.handleRenderEDL(mustTask(t, s, "t_ghost"))
	ghost := mustTask(t, s, "t_ghost")
	if ghost.Status != StatusError || ghost.RetryCount != 0 || ghost.CooldownReason != errCodeNodeNotFound {
		t.Fatalf("节点不存在应直接失败(E_NODE_NOT_FOUND): %+v", ghost)
	}

	// 未指定节点 → B-08 自动选机（在线且空闲的 srv1），落 server_id 并进入 RUNNING。
	s.UpsertTask(Task{ID: "t_noserver", OrderID: 3, Status: StatusQueue, TaskType: TaskTypeRenderEDL,
		PayloadJSON: `{"clips":[]}`})
	sch.handleRenderEDL(mustTask(t, s, "t_noserver"))
	auto := mustTask(t, s, "t_noserver")
	if auto.Status != StatusTranscoding || auto.ServerID != "srv1" || auto.ServerName != "节点一" {
...
func TestB05SideGuards(t *testing.T) {
	...
	s.UpsertTask(Task{ID: "t_nopayload", OrderID: 1, Status: StatusQueue, TaskType: TaskTypeRenderEDL, ServerID: "srv1"})
	sch.handleRenderEDL(mustTask(t, s, "t_nopayload"))
	empty := mustTask(t, s, "t_nopayload")
	if empty.Status != StatusError || empty.RetryCount != 0 || empty.CooldownReason != errCodePayloadMissing {
		t.Fatalf("载荷缺失应直接失败(E_PAYLOAD_MISSING): %+v", empty)
	}

	// 2) 下发通道未注入（B-06 未到）→ 保持 QUEUE，不得误置 RUNNING。
	sch2, s2 := newSchedTestEnv(t)
	s2.UpsertTask(Task{ID: "t_wait", OrderID: 1, Status: StatusQueue, TaskType: TaskTypeRenderEDL,
		ServerID: "srv1", PayloadJSON: `{"clips":[]}`})
	sch2.handleRenderEDL(mustTask(t, s2, "t_wait"))
	if got := mustTask(t, s2, "t_wait"); got.Status != StatusQueue || got.RemoteTaskID != "" {
	...
	sch2.processTask(Task{ID: "t_cd", Status: StatusCooldown, TaskType: TaskTypeRenderEDL})
	if d.edlCalls != 0 || d.proxyCalls != 0 {
		t.Fatalf("COOLDOWN 任务不应触发下发")
	}
	sch2.processTask(Task{ID: "t_run", Status: StatusTranscoding, TaskType: TaskTypeRenderEDL})
	if d.edlCalls != 0 {
